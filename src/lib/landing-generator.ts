...
export function generateLandingTSX(config: any): string {
  ...
  const bannerJSX = config.bannerUrl
    ? `<div className="w-full">
         <img src="${config.bannerUrl}" alt="Banner principal" className="w-full object-cover" />
       </div>`
    : "";
  ...
  const sectionsJSX = (config.sections || [])
    .map((sec: SectionFull) => {
      // Cada sección tiene: id, type, title, content, order, sectionKey?, mainImageId?, images?
      const imagesInline = (sec.images || [])
        .map((img: ImageFull) => `
          <img
            ...
            src="${img.url}"
            alt="${img.altText || ""}"
            className="w-full max-w-md object-cover rounded shadow mb-4"
          />
        `)
        .join("");
  ...
  const articlesJSX = (config.articles || [])
    .map((art: ArticleFull) => {
      ...
      const enlace = art.url
        ? `<a href="${art.url}" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
  ...
  // --- Imágenes Globales (no asociadas a sección) ---
  ...
  // --- Pasos Legales (usa cualquier “shape” que venga en config.legalSteps) ---
  const legalStepsJSX = (config.legalSteps || [])
    .map((step: any, idx: number) => {
      const icon = step.iconUrl
        ? `<img src="${step.iconUrl}" alt="Icono paso ${idx + 1}" className="w-8 h-8 mr-2" />`
        : "";
      return `
  ...
  // --- Enlaces de Footer (usa cualquier “shape” que venga en config.footerLinks) ---
